...
void MediaCaptureDemoSystem::UploadPreviewFrame(PreviewTexture &preview, const uint8_t *data, uint32_t dataSize, uint32_t width, uint32_t height)
{
	...
	{
		if (!CreatePreviewTexture(preview, width, height))
			return;
	}
	...
	if (FAILED(preview.uploadBuffer->Map(0, &readRange, reinterpret_cast<void **>(&mapped))))
		return;
...
void MediaCaptureDemoSystem::DrawMicrophonePanel()
{
	...
	{
		ImGui::InputText("Model Path", whisperModelPath_.data(), whisperModelPath_.capacity() + 1, ImGuiInputTextFlags_CallbackResize, [](ImGuiInputTextCallbackData *data) -> int
						 {
		...
		if (showDetailedResult_ && !detailedResult_.segments.empty())
		{
			for (int si = 0; si < static_cast<int>(detailedResult_.segments.size()); ++si)
			{
				...
				if (ImGui::TreeNode(reinterpret_cast<void *>(static_cast<intptr_t>(si)), "Tokens (%d)", static_cast<int>(seg.tokens.size())))
				{
					...
					{
						ImGui::TextWrapped("%s", tok.text.c_str());
						ImGui::NextColumn();
						ImGui::Text("%.2f%%", tok.probability * 100.0f);
						ImGui::NextColumn();
						ImGui::Text("%.3f", tok.voiceLength);
						ImGui::NextColumn();
						ImGui::Text("%.2f-%.2f", tok.t0 * 0.01f, tok.t1 * 0.01f);
						ImGui::NextColumn();
					}
					ImGui::Columns(1);
					ImGui::TreePop();
...
void MediaCaptureDemoSystem::DrawVoiceVoxPanel()
{
	...
	ImGui::InputText("Engine Path", voiceVoxEnginePath_.data(), voiceVoxEnginePath_.capacity() + 1, ImGuiInputTextFlags_CallbackResize, [](ImGuiInputTextCallbackData *data) -> int
					 {
			if(data->EventFlag == ImGuiInputTextFlags_CallbackResize){
				auto* str = static_cast<std::string*>(data->UserData);
				str->resize(data->BufTextLen);
	...
	{
		std::string selectedLabel = voiceVoxSpeakers_[selectedSpeaker_].name + " (" + voiceVoxSpeakers_[selectedSpeaker_].styleName + ")";
		if (ImGui::BeginCombo("Speaker", selectedLabel.c_str()))
		{
			for (int i = 0; i < static_cast<int>(voiceVoxSpeakers_.size()); ++i)
			{
				std::string label = voiceVoxSpeakers_[i].name + " (" + voiceVoxSpeakers_[i].styleName + ")";
	...
	// Text input
	ImGui::InputTextMultiline("##voicevox_text", voiceVoxText_.data(), voiceVoxText_.capacity() + 1, ImVec2(-1, 80), ImGuiInputTextFlags_CallbackResize, [](ImGuiInputTextCallbackData *data) -> int
							  {
			if(data->EventFlag == ImGuiInputTextFlags_CallbackResize){
				auto* str = static_cast<std::string*>(data->UserData);
				str->resize(data->BufTextLen);
...
void MediaCaptureDemoSystem::DrawVisionPanel()
{
	...
	// API Key input
	ImGui::InputText("API Key", visionApiKey_.data(), visionApiKey_.capacity() + 1, ImGuiInputTextFlags_Password | ImGuiInputTextFlags_CallbackResize, [](ImGuiInputTextCallbackData *data) -> int
					 {
			if(data->EventFlag == ImGuiInputTextFlags_CallbackResize){
				auto* str = static_cast<std::string*>(data->UserData);
				str->resize(data->BufTextLen);
	...
	// Analyze button
	bool hasSource = (visionSource_ == 0 && webCamera_ && webCamera_->IsCapturing()) || (visionSource_ == 1 && screenCapture_ && screenCapture_->IsCapturing());
